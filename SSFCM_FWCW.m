function [Cluster_elem,M,W,Z]=SSFCM_FWCW(X,M,k,p_init,p_max,p_step,t_max,beta_memory,N,fuzzy_degree,d,q,landa,f,b,alpha)
%
%[Cluster_elem,M,W,Z]=SSFCM_FWCW(X,M,k,p_init,p_max,p_step,t_max,beta_memory,N,fuzzy_degree,d,q,landa)
%
%This demo shows how to call the SSFCM-FWCW algorithm described in the paper:
%Amin Golzari Oskouei, Negin Samadi, and Jafar Tanha, "SSFCM-FWCW: Semi-Supervised Fuzzy C-means
...
while 1
    ...
    Cluster_elem = transpose( (1/(1+alpha)) * (((1 + alpha - (alpha*b))./tmp1)+ (alpha*b.*f) ) );
    ...
    O_F=object_fun(N,d,k,Cluster_elem,landa,M,fuzzy_degree,W,Z,q,p,X,f,b, alpha);
    ...
    for j=1:k
        distance(j,:,:) = (1-exp((-1.*repmat(landa,N,1)).*((X-repmat(M(j,:),N,1)).^2)));
        dWkm(j,:) = ((Cluster_elem(j,:).^fuzzy_degree)+(alpha * ((Cluster_elem(j,:)-(b.*f(:,j))').^fuzzy_degree))) * reshape(distance(j,:,:),[N,d]);
    end
    ...
    for j=1:k
        ...
        WBETA = transpose(Z(j,:).^q);
        WBETA(WBETA==inf)=0;
        Dw(1,j) = transpose(WBETA) * transpose(reshape(distance(j,:,:),[N,d])) *  ((transpose(Cluster_elem(j,:).^fuzzy_degree)) + (alpha *(transpose((Cluster_elem(j,:)-(b.*f(:,j))').^fuzzy_degree))));
    end
